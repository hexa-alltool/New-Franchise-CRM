...
export default function BookOrders() {
  ...
  async function updateStatus(id,status){ await supabase.from('book_orders').update({status,updated_at:new Date().toISOString()}).eq('id',id); await fetchOrders(); setDetail(null) }

  const columns=[
    {key:'franchise',label:'Franchise',render:(v,row)=><div><p className="font-medium text-sm">{v?.name}</p><p className="text-xs text-gray-400">{v?.city}</p></div>},
    {key:'order_type',label:'Type',render:v=><Badge label={v?.replace('_',' ')} variant="info"/>},
    {key:'quantity',label:'Qty',render:v=><span className="font-medium">{v}</span>},
    ...
    <div className="p-4 sm:p-6 space-y-4">
      ...
      <DataTable columns={columns} data={orders} loading={loading} searchKeys={[]} emptyText="Koi order nahi"
        actions={row=>row.status==='pending'?(
          <>
            <button onClick={()=>updateStatus(row.id,'approved')} className="p-1.5 rounded text-gray-400 hover:bg-green-50 hover:text-green-600"><Check className="w-3.5 h-3.5"/></button>
            <button onClick={()=>updateStatus(row.id,'rejected')} className="p-1.5 rounded text-gray-400 hover:bg-red-50 hover:text-red-600"><X className="w-3.5 h-3.5"/></button>
          </>
        ):<button onClick={()=>setDetail(row)} className="text-xs text-primary-600 hover:underline">Update</button>}
      />
      <Modal open={!!detail} onClose={()=>setDetail(null)} title="Update order" size="sm">
        {detail&&<div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2">
            {['approved','dispatched','delivered','rejected'].map(s=>(
              <button key={s} onClick={()=>updateStatus(detail.id,s)} className={`py-2 rounded-lg text-xs font-medium border transition-colors ${detail.status===s?'bg-primary-600 text-white border-primary-600':'bg-white text-gray-700 border-gray-200 hover:bg-gray-50'}`}>
